...
public class OriginalSchwartzHearst {
    ...
    public String extractBestLongForm(String shortForm, String longForm) {
	String bestLongForm;
	StringTokenizer tokenizer;
	int longFormSize, shortFormSize;
    ...
    private void newExtractAbbrPair(String shortForm, String longForm, Map<String,String> candidates) {
	String bestLongForm;
	StringTokenizer tokenizer;
	int longFormSize, shortFormSize;
	...
	if (testMode) {
	    ...
	    //System.out.println(shortForm + delimiter + bestLongForm);
	    candidates.put(shortForm,bestLongForm);
	}
    }
    
    public Map<String,String> extractAbbrPairsFromStr(String text) {
	...
	try {
		...
		do {
			...
			if (shortForm.length() > 1 && longForm.length() > 1) {
			    ...
			    if (isValidShortForm(shortForm)){
			    	newExtractAbbrPair(shortForm.trim(), longForm.trim(), candidates);
			    }
			}
			currSentence = currSentence.substring(closeParenIndex + 1);
		    } else if (openParenIndex > -1) {
	...
	OriginalSchwartzHearst extractAbbrev = new OriginalSchwartzHearst();
	Vector candidates;	
	String[] candidate;
